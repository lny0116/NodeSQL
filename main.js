...
var app = http.createServer(function(request,response){
    ...
    if(pathname === '/'){
      ...
      request.on('end', function(){
          var post = qs.parse(body);
          db.query(`UPDATE topic SET title=?, description=?, author_id=? WHERE id=?`, [post.title, post.description, post.author, post.id], function (error, result){ //저자값을 1로 햇던걸 ?로 post형식으로 바꿈.
